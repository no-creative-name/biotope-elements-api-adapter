import { getObjectForContentId, getSubItemsForMainLocation } from './helpers/cmsHelpers';

export const getModules = async (pageId) => {
	const pageData = await getObjectForContentId(pageId);
	const subItems = await getSubItemsForMainLocation(pageData.Content.MainLocation._href);
	let contentIds = [];
	subItems.map((subItem: any) => {
		contentIds.push(subItem.Content._href.split(/[\s/]+/).pop());
	});

	return await Promise.all(contentIds.map(async (contentId: any) => {
