...
export const generateWebComponentTags = (child: any) => {
    ...
    return `${scaffolding.before}<${generateComponentNameFromString(metaData.componentName)}
        data-resources="[
            {
    ...
    >${renderChildren(data)}</${generateComponentNameFromString(metaData.componentName)}>${scaffolding.after}`
};