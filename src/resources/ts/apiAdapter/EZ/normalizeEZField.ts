import { getContentTypeAndId, getLocation, getSystemUrl, getContentForContentId, getFieldFromName, convertTimestampToDDMMYYYY, convertRichTextToJSON, getContentTypeObjectForContentId, getContentId, getObjectForContentId } from "../../helpers/cmsHelpers";

export const normalizeEZField = async (ezFieldType: string, contentObject: any, fieldName: string, fieldValue: any, pageId: number) => {
    switch(ezFieldType) {
        case 'ezselection':
            ...
            let correctField = contentFieldInfo.find((field) => {
                return (fieldName === field.identifier && field.fieldSettings.options[fieldValue])
            })
        ...
        case 'ezobjectrelationlist':
            if(fieldValue.destinationContentIds.length) {
                const locations = await getLocation(fieldValue.destinationContentIds[0]);
                ...
                for(const locationStep of reversedLocations) {
                    if(parseInt(locationStep)) {
                        ...
                        if(contentId != 0) {
                            const contentType = (await getContentTypeAndId(await getObjectForContentId(contentId))).contentType;
                            if(contentType == 'page') {
        ...
        case 'ezimageasset':
            if(fieldValue.destinationContentId != null) {
                ...
                const image = {
                    url: `${EZAPI}${getFieldFromName(imageObject, 'image').fieldValue.uri}`,
