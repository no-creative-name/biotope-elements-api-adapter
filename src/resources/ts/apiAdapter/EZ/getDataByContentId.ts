...
const getDataByContentId = async (
  ...
  await Promise.all(
    contentFields.map(async (contentField: any) => {
  ...
  const contentSubItems = await getSubItemsForMainLocation(
    contentObject.Content.MainLocation._href
  );

  const childrenContent = await Promise.all(
    contentSubItems.map(async (subItem: any, index: number) => {
