...
export const getFieldFromName = (context, name) =>
  context.find((element: any) => element.fieldDefinitionIdentifier === name);

export const getSystemUrl = async (location: string) => {
  let url = `${CMSAPI}/api/ezp/v2/content/locations/1/2/${location}/urlaliases?custom=false`;
...
export const getContentId = (locationId: number) => {
  const url = `${CMSAPI}/api/ezp/v2/content/locations?id=${locationId}`;
  return fetch(url, {
    headers: {
    ...
    .then(res => {
      if (!res.ok) {
        console.warn(
          "Content on location not found, please check internal links"
        );
      } else {
        return res;
      }
...
export const getButtonData = async button => {
  ...
  if (type == LINK_TYPES.JUMPMARK) {
    ...
    url =
      "#jumpmark-" +
      getFieldFromName(button, "module_object").fieldValue.destinationContentId;
  } else if (type == LINK_TYPES.INTERNAL) {
    const referenceId = getFieldFromName(button, "page_object").fieldValue
    ...
    target = getFieldFromName(button, "target_blank").fieldValue
      ? "_blank"
      : "";
  }
    ...
    data.map(async child => {
        ...
        await Promise.all(
          child.attributes.map(async attribute => {
            ...
            if (attribute.value.indexOf("ezlocation") != -1) {
              ...
              const contentType = (await getContentTypeAndId(linkedContent))
                .contentType;
              if (contentType == "file") {
      ...
      return {
        ...
        ...(child.children !== undefined && {
          children: await beautifyJSON(child.children)
        })
      };
    })
  );
...
export const resolveLink = async (data: any, otps: any) => {
  ...
  if (link === null) {
    const rel = getFieldFromName(data, options.linkInternal).fieldValue
