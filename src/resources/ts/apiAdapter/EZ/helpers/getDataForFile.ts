import { getFieldFromName, getObjectForContentId } from "./ezHelpers";

export const getDataForFile = async (contentId: number) => {
  const contentObject = await getObjectForContentId(contentId);
  const contentFields =
    contentObject.Content.CurrentVersion.Version.Fields.field;
  ...
  if (getFieldFromName(contentFields, "file").fieldValue !== null) {
    ...
    getFieldFromName(contentFields, "multilingual_file").fieldValue !== null
  ) {
  ...
  let linkTitle = getFieldFromName(contentFields, "description").fieldValue
    .xhtml5edit;
